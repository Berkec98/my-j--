...
enum TokenKind {
    ...
    VOID("void"), WHILE("while"), BREAK("break"), CONTINUE("continue"), DOUBLE("double"),
    FOR("for"), LONG("long"), THROWS("throws"), CASE("case"),DEFAULT("default"),
    FINAL("final"), IMPLEMENTS("implements"), SWITCH("switch"), TRY("try"), CATCH("catch"),
    DO("do"), FINALLY("finally"), INTERFACE("interface"), THROW("throw"),

    // Operators.
    ASSIGN("="), DEC("--"), EQUAL("=="), LT("<"), GT(">"), GE(">="), INC("++"), LAND("&&"),
    LOR("||") , LE("<="), LNOT("!"),
    MINUS("-"), MINUS_ASSIGN("-=") ,PLUS("+"), PLUS_ASSIGN("+="), STAR("*"), STAR_ASSIGN("*="),
    DIV("/"), DIV_ASSIGN("/="), REM("%"), REM_ASSIGN("%="),
    ARSHIFT(">>"), ARSHIFT_ASSIGN(">>="), ALSHIFT("<<"), ALSHIFT_ASSIGN("<<="), LRSHIFT(">>>"),
    LRSHIFT_ASSIGN(">>>="), OR("|"), OR_ASSIGN("|="),
    XOR("^"), XOR_ASSIGN("^="), AND("&"), AND_ASSIGN("&="),
    QUESTION("?"), COLON(":"), NOT("~"), NOT_EQUAL("!="),
