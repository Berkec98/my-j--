...
public class Parser {
    ...
    private JStatement statement() {
        ...
        if (see(LCURLY)) {
            ...
            ArrayList<JStatement> initials = new ArrayList<JStatement>();
            ...
            return new JForStatement(line, initials, condition, updates, body);
        }
        else if (have(IF)) {
    ...
    private ArrayList<JStatement> forInit() {
        ArrayList<JStatement> initExps = new ArrayList<JStatement>();
        if (seeLocalVariableDeclaration()) {
            int line = scanner.token().line();
            ArrayList<JVariableDeclarator> vdecls = variableDeclarators(type());
            JStatement jVari = new JVariableDeclaration(line, vdecls);
            initExps.add(jVari);
        } else {
            do{
                initExps.add(statementExpression());
            }while(see(COMMA));
        }
        return initExps;
    }
